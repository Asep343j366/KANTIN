...
export async function finalizeSubscriptionOrder(db, orderId, { amountCharged } = {}) {
  ...
  // Idempotensi dijamin gerbang UPDATE di atas (hanya 1 pemanggil menang), jadi
  // jurnal cukup di-insert sekali. sub_order_id hanya utk telusur — bila kolomnya
  // belum ada (mis. migrasi v15 belum lengkap), fallback insert tanpa kolom itu
  // agar pemasukan TETAP tercatat.
  ...
  if (plat?.id) {
    ...
    let jErr = (await db.from("journal").insert({ ...base, sub_order_id: won.id })).error;
    if (jErr) {
      // Kemungkinan kolom sub_order_id belum ada → coba lagi tanpa kolom itu.
      const retry = await db.from("journal").insert(base);
      jErr = retry.error;
    }
    if (jErr) console.error("[subscription finalize] gagal catat jurnal:", jErr.message || jErr);
  } else {
    console.error("[subscription finalize] store platform (is_platform_admin) tidak ditemukan — jurnal dilewati.");
  }
