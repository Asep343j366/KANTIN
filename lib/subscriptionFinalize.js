...
export async function finalizeSubscriptionOrder(db, orderId, { amountCharged } = {}) {
  ...
  // 4) Jurnal — dua sisi (idempotensi dijamin gerbang UPDATE di atas):
  //    a) PEMASUKAN di J366 (platform), b) PENGELUARAN di store pembeli.
  // sub_order_id hanya utk telusur; bila kolomnya belum ada (migrasi v15 belum
  // lengkap), fallback insert tanpa kolom agar entri TETAP tercatat.
  ...
  const insertJurnal = async (row) => {
    let err = (await db.from("journal").insert({ ...row, sub_order_id: won.id })).error;
    if (err) err = (await db.from("journal").insert(row)).error; // fallback tanpa sub_order_id
    if (err) console.error("[subscription finalize] gagal catat jurnal:", err.message || err);
  };
  ...
  if (plat?.id) {
    ...
    console.error("[subscription finalize] store platform tidak ditemukan — jurnal pemasukan dilewati.");
  }

  // b) Pengeluaran store pembeli (biaya berlangganan)
  await insertJurnal({
    store_id: won.store_id,
    jenis: "keluar",
    kategori: "Langganan",
    keterangan: `Biaya langganan ${won.durasi_hari} hari`,
    jumlah: won.amount,
    dicatat_oleh: "sistem",
  });
